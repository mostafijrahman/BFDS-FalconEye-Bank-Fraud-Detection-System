...
-- ============================================================
-- ARCHITECTURE:
--   fn_HaversineDistance   — Scalar function: great-circle km
--   usp_DetectGeoImpossibility — SP: physics analysis, OUTPUT params
--   trg_GeoImpossibilityAlert  — AFTER INSERT trigger:
--                                  1. Reverses suspicious transaction
--                                  2. Raises GEO_IMPOSSIBLE alert
--                                  3. Writes transaction_audit_log
--
-- DETECTION THRESHOLD: 900 km/h (commercial airline speed ceiling)
-- ============================================================
...
BEGIN
    ...
    -- ── STEP 1: Find the most recent prior transaction with coordinates
    SELECT TOP 1
    ...
    SET @InvestigationNotes =
        ...
        + '============================================================'
        + CHAR(13)+CHAR(10)
        + 'LOCATION A — Previous Transaction'
        + CHAR(13)+CHAR(10)
        ...
        + '------------------------------------------------------------'
        + CHAR(13)+CHAR(10)
        + 'LOCATION B — Suspicious Transaction (status = reversed)'
        + CHAR(13)+CHAR(10)
...
-- ============================================================
-- For every INSERT on transactions:
--   1. Calls usp_DetectGeoImpossibility via OUTPUT params
--   2. On violation:
--      A) Inserts a 'reversed' evidence row into transactions
--      B) Inserts (or updates) a GEO_IMPOSSIBLE alert
--      C) Writes one entry per row into transaction_audit_log
--   3. On clean pass:
--      C) Still writes a 'no_violation' entry into transaction_audit_log
-- ============================================================
...
BEGIN
    ...
    DECLARE @ReversedTxnID      BIGINT;

    -- ── Cursor: one row per inserted record with coordinates ─
    ...
    BEGIN
        ...
        BEGIN
            ...
            -- ACTION A: Insert 'reversed' evidence transaction row
            ...
            VALUES
            (
                @CardID, @MerchantID, @UserID,
                @Amount, @Currency, @TxnTimestamp,
                @MerchantCountry, @CustomerCountry, @MerchantMCC,
                'reversed',                          -- geo violation evidence
                @IsOnline, @DeviceID, @IPAddress, @Lat, @Lon
            );

            SET @ReversedTxnID = SCOPE_IDENTITY();

            -- ACTION B: Insert or Update GEO_IMPOSSIBLE alert
            IF EXISTS (
                SELECT 1 FROM alerts
                WHERE  user_id     = @UserID
                  AND  alert_type  = 'GEO_IMPOSSIBLE'
            ...
            BEGIN
                ...
                SET
                    ...
                    investigation_notes =
                        ...
                        + CHAR(13)+CHAR(10)
                        + 'New reversed txn_id : ' + CAST(@ReversedTxnID AS NVARCHAR(20))
                        + CHAR(13)+CHAR(10)
                        + @InvestigationNotes
                WHERE  user_id     = @UserID
                  AND  alert_type  = 'GEO_IMPOSSIBLE'
            ...
            BEGIN
                ...
                VALUES
                (
                    @ReversedTxnID, @CardID, @UserID,
                    'GEO_IMPOSSIBLE', @RiskScore, @AlertMessage,
                    'open', @InvestigationNotes, GETUTCDATE(), NULL
                );
            END

            -- ACTION C: Write VIOLATION audit log entry
            ...
            (
                ...
                + 'Original txn_id: '   + CAST(@TxnID AS NVARCHAR(20))
                + ' | Distance: '       + CAST(@DistanceKm AS NVARCHAR(20))           + ' km'
                + ' | Time Gap: '       + CAST(@TimeDiffMinutes AS NVARCHAR(20))       + ' mins'
                + ' | Req. Speed: '     + CAST(CAST(@RequiredSpeedKmh AS INT) AS NVARCHAR(20)) + ' km/h'
                + ' | Speed Ratio: '    + CAST(@SpeedRatio AS NVARCHAR(20))            + 'x'
        ...
        BEGIN
            ...
            (
                ...
                + CASE
                    ...
                    ELSE
                        'Distance: '   + CAST(@DistanceKm AS NVARCHAR(20))   + ' km'
                        + ' | Speed: ' + CAST(CAST(@RequiredSpeedKmh AS INT) AS NVARCHAR(20)) + ' km/h'
                        + ' — within ' + CAST(CAST(900 AS INT) AS NVARCHAR(10)) + ' km/h limit.'
                  END
            );
        END
...
--    Expected: 1 completed row | 0 alerts | audit log: no violation
-- ------------------------------------------------------------
...
PRINT 'TEST 1 DONE — Expected: 1 completed row | 0 alerts | audit: no violation.';
GO

-- ❌ TEST 2: IMPOSSIBLE — User 1: Shreveport → Moscow (seconds later)
--    Distance ≈ 9,800 km | CRITICAL speed ratio
--    Expected: original completed + new reversed row | GEO_IMPOSSIBLE alert | audit log entry
-- ------------------------------------------------------------
...
PRINT 'TEST 2 DONE — Expected: reversed row + GEO_IMPOSSIBLE alert + audit log entry.';
GO

-- ❌ TEST 3: IMPOSSIBLE — User 2: Dallas → Tokyo (seconds later)
--    Distance ≈ 11,350 km | CRITICAL
--    Expected: original completed + new reversed row | new GEO_IMPOSSIBLE alert for user 2
-- ------------------------------------------------------------
...
PRINT 'TEST 3 DONE — Expected: reversed row + new GEO_IMPOSSIBLE alert (user 2).';
GO

-- ❌ TEST 4: REPEAT VIOLATION — User 1: Moscow → Sydney (seconds later)
--    Distance ≈ 14,500 km | Existing open alert should be UPDATED, not duplicated
-- ------------------------------------------------------------
PRINT '==== TEST 4: Repeat Violation — User 1: Moscow → Sydney ====';
...
PRINT 'TEST 4 DONE — Expected: reversed row + EXISTING alert UPDATED for user 1.';
GO

-- ✅ TEST 5: SAFE — User 5 first transaction in Houston (no prior geo point)
--    Expected: 1 completed row | 0 alerts | audit log: no prior coordinates
-- ------------------------------------------------------------
...
PRINT 'TEST 5 DONE — Expected: 1 completed row | 0 alerts | audit: no prior coords.';
GO
...
-- All transactions — completed originals + reversed fraud evidence rows
SELECT
    transaction_id,
    user_id,
    card_id,
    ...
    merchant_country,
    ip_address,
    CAST(latitude AS NVARCHAR(15)) + ', ' + CAST(longitude AS NVARCHAR(15)) AS coordinates,
    transaction_status,
...
-- ============================================================
-- END OF RULE 1: Geographic Impossibility
-- ============================================================
